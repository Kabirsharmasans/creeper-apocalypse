...
public class DayChangeHandler {
    ...
    private static final int BLOOD_MOON_DAY = 5;
    private static final int CHARGED_DAY = 7;
    private static final int SWARM_DAY = 10;
    private static final int SUPER_MILESTONE_INTERVAL = 5;
    private boolean registered;

    public void register() {
        if (registered) {
            CreeperApocalypse.LOGGER.debug("Day change handler already registered; skipping duplicate registration");
            return;
        }
    ...
    private void checkMilestones(MinecraftServer server, int day) {
        ...
        if (day == BLOOD_MOON_DAY && CreeperApocalypse.CONFIG.bloodMoonEnabled()) {
            announceBloodMoon(server);
        }

        if (day == CHARGED_DAY && CreeperApocalypse.CONFIG.chargedDayEnabled()) {
            announceChargedDay(server);
        }

        if (day == SWARM_DAY && CreeperApocalypse.CONFIG.swarmDayEnabled()) {
            announceSwarm(server);
        }

        if (day > SWARM_DAY && day % SUPER_MILESTONE_INTERVAL == 0) {
    ...
    private void announceBloodMoon(MinecraftServer server) {
        Text announcement = Text.literal("")
            .append(Text.literal("[DANGER] ").formatted(Formatting.DARK_RED))
    ...
    private void announceChargedDay(MinecraftServer server) {
        Text announcement = Text.literal("")
            .append(Text.literal("[POWER] ").formatted(Formatting.AQUA))
    ...
    private void announceSwarm(MinecraftServer server) {
        Text announcement = Text.literal("")
            .append(Text.literal("[SWARM] ").formatted(Formatting.GREEN))
    ...
    private void announceSuperMilestone(MinecraftServer server, int day) {
        Text announcement = Text.literal("")
            .append(Text.literal("[MILESTONE] ").formatted(Formatting.GOLD))
